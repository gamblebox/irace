...
switch ($parameter) {
	...
	case 'close':
		while (TRUE) {
			$sql = "select * from race where start_time < DATE_ADD(NOW(), INTERVAL 0 SECOND) AND start_time > DATE_ADD(NOW(), INTERVAL -100 SECOND) AND association_code = 'powerball' AND stat = 'P' order by start_time desc LIMIT 1";
			...
			if (count($close_game) > 0) {
				...
				do {
					...
					echo date("Y-m-d H:i:s") . ' sleep 5 for try' . PHP_EOL;
					sleep(5);
				} while ($close_game[0]->own_race_no != $result->times && $count < 10);
				print_r($result);
	...
	case 'full':
		$url = 'http://ntry.com/stats/api.php?offset=0&size=20&cmd=get_round_data&type=powerball&get_type=round&get_value=20';
		echo $url . PHP_EOL;
