...
function get_curl_full($url, $url_json, $post_data)
{
	...
	// curl_setopt($ch, CURLOPT_REFERER, 'https://autorace.jp/race/Program/isesaki/2022-04-07_3');
	...
	// $post_data['race_session'] = $cookies['race_session'];
	...
	//리퍼러
	curl_setopt($ch, CURLOPT_REFERER, 'https://autorace.jp/race/Live/isesaki');
...
function get_race_data_to_json($place)
{
	...
	$result = get_curl_json('https://autorace.jp/race/OtherRaceInfo', $post_data, $cookies);
	// print_r($result);
	...
	for ($i = 1; $i < $finalRaceNo + 1; $i++) {
		...
		$result = get_curl_json('https://autorace.jp/race/Program', $post_data, $cookies);
		$json = json_decode($result['contents']);
		if ($json->result != 'Success') {
			return;
		}
