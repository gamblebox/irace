...
//temp
//$open_time = $today . ' 08:37:50';
//$own_race_no = 955609;
//$start_time = $open_time;
//$start_date = $tomorrow;

$sql = "INSERT INTO race (association_code, place_id, own_race_no, race_no, start_date, start_time) SELECT 'powerball', 0, ?, ?, ?, ? FROM DUAL WHERE NOT EXISTS ( SELECT * FROM race WHERE race_no = ? AND start_date = ?)";
for ($i = 0; $i < 288; $i++) {
	// 	echo interpolateQuery($sql, array($game_code, $game_own_id, $i+1, $game_start_time)) . PHP_EOL;
	$stmt = $db->prepare($sql);
	$msg = $stmt->execute(array($own_race_no, $i + 1, $start_date, $start_time, $race_no, $start_date));
	// echo $own_race_no . ':' . $start_time . '=>' . ($msg ? 'Exec Ok' : $msg) . PHP_EOL;
	echo $i + 1 . ' : ' . $own_race_no . ':' . $start_time . '=>' . $msg . PHP_EOL;
	// $own_race_no++;
	// $own_race_no = $own_race_no + 600;
