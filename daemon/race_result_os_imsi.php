...
$race_day = date('Ymd', strtotime(date(Ymd) . '+' . '-1' . ' days')); // 1일 후      
echo '$race_day ' . $race_day . PHP_EOL;
...
$sql = "SELECT * FROM race WHERE  race.start_date = '2019-03-01' and race.association_code in ('osr','osh','osg') and stat = 'P' and place_name = 'LINGFIELD' order by start_time asc;";
$race = select_sql($sql);
print_r($race);
//https://www.tab.com.au/racing/2019-02-26/KENILWORTH/J/R/3
if ($race) {
    //         $url = 'https://www.tab.com.au/racing/2019-01-16/CANNINGTON/W/G/9/Win';
    //         get_race_data_to_json($casper, $url, $v->id, $v->place_code, $v->race_no, $v->association_code);
    foreach ($race as $i => $v) {
        //$url = 'https://www.tab.com.au/racing/' . $v->start_date . '/' . $v->place_name . '/E' . '/' . strtoupper(str_replace('os', '', $v->association_code)) . '/' . $v->race_no;
        $url = 'https://www.tab.com.au/racing/' . $v->start_date . '/' . $v->place_name . '/E' . '/' . 'H' . '/' . $v->race_no;
        echo $url . PHP_EOL;
        try {
            get_race_data_to_json($casper, $url, $v->id, $v->place_code, $v->race_no, $v->association_code);
        } catch (Exception $e) {
