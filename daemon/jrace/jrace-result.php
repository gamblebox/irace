...
function getRaceData($race)
{
    ...
    //https://www2.keiba.go.jp/KeibaWeb/TodayRaceInfo/RefundMoneyList?k_raceDate=2018%2f06%2f03&k_babaCode=10
    $url = 'https://www2.keiba.go.jp/KeibaWeb/TodayRaceInfo/RefundMoneyList?k_raceDate=' . str_replace('-', '%2f', $date) . '&k_babaCode=' . $place_own_id;
    ...
    foreach ($divs as $index => $div) {
        ...
        foreach ($trs as $index => $tr) {
            ...
            if ($temp_type == '枠連複' || $temp_type == '枠連単') {
                continue;
            }
            if ($temp_type != '') {
                $type = $temp_type;
...
$sql = "
    SELECT 
        ...
        place.own_id 
    FROM race 
        left join place on ( race.place_id = place.id) 
    WHERE
        race.start_time < now()
        and race.start_date = '" . $race_day . "' 
        and race.association_code = 'jrace' 
        and stat = 'P' 
    group by place.own_id 
...
foreach ($data as $i => $r) {
    ...
    $race_no = $r['race_no'];
    $sql = 'SELECT id, place_id, race_no FROM `race` WHERE start_date = date(' . '\'' . $start_date . '\') and (SELECT id from place where own_id = \'' . $own_id . '\') = place_id and race_no = \'' . $race_no . '\'';
    print_r($sql);
    $raceInfo = select_query_one($database, $sql);
    // print_r( $v[0]->id);
    $race_id = $raceInfo->id;
    $place_id = $raceInfo->place_id;
    ...
    if (count($results) > 0) {
        ...
        $result = select_query_one($database, $sql);
        $odds_all = $result->odds_all;
        ...
        $sql = "
            INSERT INTO isports.announces (association_code, race_id, type, memo)
            SELECT 'jrace', {$race_id}, '경주결과등록', CONCAT(name, ' ', '{$race_no}', '경주 결과 등록 되었습니다')
            FROM place
            WHERE id = {$place_id};
        ";
        print_r($sql);
