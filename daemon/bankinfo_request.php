...
Editor::inst( $db, 'bbs_qna' )->fields(
		//Field::inst( 'banking_type' ),
		Field::inst( 'bbs_qna.user_id' )->options( 'user_info', 'user_id', array( 'nick_name', 'club_user_id' ), function ( $q ) {
			global $club_id;
			$q->where( 'club_id', $club_id, ( $club_id === 255 ? '!=' : '=' ) );
		}, function ( $row ) {
			return $row[ 'club_user_id' ] . '(' . $row[ 'nick_name' ] . ')';
		} ),
		Field::inst( 'bbs_qna.club_id' ),
		//Field::inst( 'club_id' ),
		...
		global $club_id;
		$editor
			->field( 'bbs_qna.club_id' )->setValue( $club_id );
	} )
