...
switch ($parameter) {
		...
		while (TRUE) {
			...
			if (count($short_games) > 0) {
				foreach ($short_games as $short_game) {
					...
					$json = file_get_contents('http://118.27.14.123/scrap.php?url=' . urlencode('http://ntry.com/stats/api.php?type=powerball&c=round&m=get_result_stream_group&date_range=' . $short_game->start_date . ',' . $short_game->start_date . '&round_range=' . $short_game->race_no . ',' . $short_game->race_no . '&group_by=reg_date&mode=powerball_odd_even'));
					echo $json;
					break;
	...
	case 'close':
		while (TRUE) {
			...
			if (count($close_game) > 0) {
				...
				do {
					$count++;
					$json = file_get_contents('http://118.27.14.123/scrap.php?url=' . urlencode('http://ntry.com/data/json/games/powerball/result.json'));
					$result = json_decode($json);
					echo $close_game[0]->own_race_no . '==' . $result->times;
					echo date("Y-m-d H:i:s") . ' sleep 5 for try' . PHP_EOL;
					sleep(3);
				} while ($close_game[0]->own_race_no != $result->times && $count < 20);
				print_r($result);
				if ($close_game[0]->own_race_no == $result->times) {
	...
	case 'full':
		$url = 'http://118.27.14.123/scrap.php?url=' . urlencode('http://ntry.com/stats/api.php?offset=0&size=20&cmd=get_round_data&type=powerball&get_type=round&get_value=20');
		echo $url . PHP_EOL;
		...
		foreach ($results as $result) {
			...
			$sql = "INSERT INTO powerball_result ( results_set, results_contents, race_id, own_race_no, race_no, stat ) SELECT ?, ?, (select id from race where start_date = ? and race_no = ?), (select own_race_no from race where start_date = ? and race_no = ?), ?, 'E' FROM DUAL WHERE NOT EXISTS ( SELECT * FROM powerball_result WHERE own_race_no =  (select own_race_no from race where start_date = ? and race_no = ?) )"; //
			// $ok = interpolateQuery($sql, array($results_set, $results_contents, $own_race_no, $own_race_no, $race_no, $own_race_no));
			$ok = interpolateQuery($sql, array($results_set, $results_contents, $start_date, $race_no, $start_date, $race_no, $race_no, $start_date, $race_no)); //
			echo $ok . PHP_EOL;
			$stmt = $db->prepare($sql);
			// $msg = $stmt->execute(array($results_set, $results_contents, $own_race_no, $own_race_no, $race_no, $own_race_no));
			$msg = $stmt->execute(array($results_set, $results_contents, $start_date, $race_no, $start_date, $race_no, $race_no, $start_date, $race_no)); //			
			echo $msg . PHP_EOL;
			// $sql = "UPDATE race SET stat = 'E' WHERE own_race_no = ? AND stat != 'E'";
			$sql = "UPDATE race SET stat = 'E' WHERE own_race_no = (select own_race_no from race where start_date = ? and race_no = ?) AND stat != 'E'"; //
			// $ok = interpolateQuery($sql, array($own_race_no));
			$ok = interpolateQuery($sql, array($start_date, $race_no)); //
			echo $ok . PHP_EOL;
			$stmt = $db->prepare($sql);
			// $msg = $stmt->execute(array($own_race_no));
			$msg = $stmt->execute(array($start_date, $race_no)); //
			echo $own_race_no . '=>' . ($msg ? 'Exec Ok' : $msg) . PHP_EOL;
		}
		break;

	case 'day':
		$json = file_get_contents('http://118.27.14.123/scrap.php?url=' . urlencode('http://ntry.com/stats/api.php?offset=0&size=288&cmd=get_round_data&type=powerball&get_type=round&get_value=288'));
		echo $json . PHP_EOL;
		$results = json_decode($json)->data;
		print_r($results);
		foreach ($results as $result) {
