...
function get_race_data_to_json($url)
{
  ...
  for ($i = 0; $i < count($heads); $i++) {
    $race_info = $xpath->query('h2', $heads[$i])[0]->textContent;
    $race_info_time = $xpath->query('p/span', $heads[$i])[0]->textContent;
    // print_r($race_info);
    //창원 제01경주
    $place = mb_substr(trim($race_info), 0, 2);
    $race_no = mb_substr(trim($race_info), -4, 2);
    $start_time = $date . ' ' . mb_substr(trim($race_info_time), -5, 5);
    $length = mb_substr(trim($race_info_time), -15, 4);
...
$alinks = ['https://www.kcycle.or.kr/contents/information/fixedChuljuPage.do'];
