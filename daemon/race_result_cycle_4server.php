...
if (isset($argv[1])) {
    require(__DIR__ . '/../../../application/configs/configdb_' . $argv[1] . '.php');
} else {
    require(__DIR__ . '/../../../application/configs/configdb.php');
}
$mysqli = new mysqli($host, $user, $password, $dbname);

$host = '127.0.0.1';
$user = 'goldrace';
$password = 'hellorhfemfpdltm^^';
$dbname = 'goldrace';
...
function get_race_data_to_json($url)
{
    ...
    // *[@id="contentForm"]/div[3]/div[2]/p[1]/strong
    //*[@id="contentForm"]/div[2]/p[1]/strong
    $date_element = $xpath->query('//*[@id="contentForm"]/div[2]/p[1]/strong');
    ...
    foreach ($trs as $index => $tr) {
        ...
        $str = trim($rowData[0]);
        $place = trim(substr($str, 0, 6));
        echo $place . PHP_EOL;

        $rowData[0] = $cycle_own_id[$place];
        echo $rowData[0] . PHP_EOL;

        $rowData[1] = $date;
        echo $rowData[1] . PHP_EOL;

        $race_no = substr($str, -2, 2);
        $rowData[2] = (int) $race_no;
        echo $rowData[2] . PHP_EOL;
...
{
    global $mysqli;
    // include (__DIR__ . '/../../../application/configs/configdb_4a.php');
...
$alinks = [
    'http://www.kcycle.or.kr/contents/information/raceResultPage.do'
];
// $alinks = ['http://www.kcycle.or.kr/contents/information/raceResultPage.do?stndYear=2020&tms_dayOrd=45-3'];
...
foreach ($data as $i => $r) {
    ...
    if (count($results) > 0) {
        ...
        $ok = insert_sql($sql);
        /*
         * $sql = "INSERT INTO `announce` (`association_code`, `type`, `memo`) VALUES ('cycle', '경주결과등록', concat((select place.name from race left outer join place on race.place_id = place.id where race.id = " . $race_id . "), ' ', (select race.race_no from race where race.id = " . $race_id . "), '경주 결과 등록 되었습니다'))";
         * $ok = insert_sql($sql);
         */
    }
