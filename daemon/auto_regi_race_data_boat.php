...
function get_race_data_to_json($url)
{
	...
	// $url = 'http://www.kboat.or.kr/contents/main/mainPage.do';
	// $url = 'http://www.kboat.or.kr/contents/information/fixedChuljuPage.do';
	...
	foreach ($races as $index => $race) {
		...
		$race = $xpath->query('h4', $head)[0]->textContent;
		$race_time = $xpath->query('p/span', $head)[0]->textContent;
		$race_length = $xpath->query('p/b', $head)[0]->textContent;
		// print_r($table);
		//창원 제01경주
		// $place = mb_substr(trim($race), 1, 2);
		$race_no = mb_substr(trim($race), 1, 2);
		$start_time = $date . ' ' . mb_substr(trim($race_time), -5, 5);
		$length = mb_substr(trim($race_length), -5, 4);
