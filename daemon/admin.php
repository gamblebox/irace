...
$partner_id = $user_id = $_SESSION['user_id'];
...
switch ($mode) {
    ...
    case 'partner_info':
        //$sql = "Select user_id, club_user_id, nick_name from `user_info` where user_level > -1 and club_id " . ($club_id ===255? '!= ': '= ') . $club_id;
        // $sql = "Select p.id, p.user_id, p.club_id, p.partner_id, p.money_point, p.partner_level, p.user_level, p.club_user_id, pc.id as partner_class_id, pc.cd_name, p.nick_name, p.phone, p.partner_calculate_rule, p.partner_calculate_ratio, p.partner_calculate_sf_win_ratio, p.partner_calculate_sf_lose_ratio, p.partner_calculate_mf_win_ratio, p.partner_calculate_mf_lose_ratio, p.r_time FROM user_info p LEFT OUTER JOIN partner_class AS pc ON p.partner_class_id = pc.id WHERE p.user_id = ?";
        $sql = "Select * FROM user_info p LEFT OUTER JOIN partner_class AS pc ON p.partner_class_id = pc.id WHERE p.user_id = ?";
    ...
    case 'partner_point':
        //$sql = "Select user_id, club_user_id, nick_name from `user_info` where user_level > -1 and club_id " . ($club_id ===255? '!= ': '= ') . $club_id;
        // 		$sql = "Select p.id, p.user_id, p.club_id, p.partner_id, p.money_point, p.partner_level, p.user_level, p.club_user_id, pc.id as partner_class_id, pc.cd_name, p.nick_name, p.phone, p.partner_calculate_rule, p.partner_calculate_ratio, p.partner_calculate_sf_win_ratio, p.partner_calculate_sf_lose_ratio, p.partner_calculate_mf_win_ratio, p.partner_calculate_mf_lose_ratio, p.r_time FROM user_info p LEFT OUTER JOIN partner_class AS pc ON p.partner_class_id = pc.id WHERE p.user_id = ?";
        $sql = "Select p.money_point FROM user_info p LEFT OUTER JOIN partner_class AS pc ON p.partner_class_id = pc.id WHERE p.user_id = ?";
        select_query($db, $sql, array($user_id));
        break;
    ...
    case 'todo':
        ...
        $sql = "select (select count(b.id)  from `banking` as b left outer join `user_info` as u on b.user_id = u.user_id where u.club_id " . ($club_id === 255 ? '!= ' : '= ') . "? and b.stat = 'R' and b.r_time >= DATE_ADD(now(),INTERVAL -2 DAY)) as banking_count,(select count(id) from `bbs_qna` where club_id " . ($club_id === 255 ? '!= ' : '= ') . "? and stat = 'R' and u_time >= DATE_ADD(now(),INTERVAL -2 DAY))  as qna_count, (select count(id) from `user_info` where club_id " . ($club_id === 255 ? '!= ' : '= ') . "? and user_level = 0)  as user_count, (select count(id) from `user_info` where club_id " . ($club_id === 255 ? '!= ' : '= ') . "? and bank_out_bank_info_ok = 'R')  as bank_confirm_count";
    ...
    case 'signage':
        ...
        $sql = "SELECT * FROM `announce` WHERE  `r_time` >= date(now()) and (`association_code`= '0' or association_code in (" . $ta . ")) and (`user_id`= '0' or `user_id`= '255' or `user_id` = ?) and (`club_id`= 255 or `club_id`= 0 or `club_id` " . ($club_id === 255 ? '!= ' : '= ') . "?" . " ) and (`tarket` = 'B' or `tarket` = 'A') " . ($club_id === 255 ? "" : " and type != '마토상담신청'")  . " order by r_time desc limit 10";
    ...
    case 'point_log':
        ...
        $sql = "SELECT * FROM `log_money` where u_time >= DATE(?) AND u_time < DATE(?) and user_id = ?";
    ...
    case 'service_log':
        ...
        $sql = "SELECT * FROM `log_money` where old_money_service != new_money_service and (memo like '손실금%' or memo like '충전%' or memo like '서비스%' or memo like '추천인%')" . " and u_time >= DATE(?) AND u_time < DATE(?) and user_id = ?";
        select_query($db, $sql, array($sdate, $edate, $user_id));
        break;

    case 'allin_service_log':
        //$sql = "SELECT * FROM `log_money` where old_money_service != new_money_service and (memo like '손실금%' or memo like '충전%' or memo like '서비스%' or memo like '추천인%')" . " and date(u_time) >= date('" . $sdate . "') and date(u_time) <= date('" . $edate . "') and user_id = '" . $user_id . "'";
        $sql = "SELECT * FROM `log_money` where old_money_service != new_money_service and memo like '손실금%'" . " and u_time >= DATE(?) AND u_time < DATE(?) and club_id = ?";
    ...
    case 'reset_user_history':
        $db->beginTransaction();
        try {
            $sql = "DELETE FROM cut_result_money WHERE club_id = ? and c_date < ?";
            $stmt = $db->prepare($sql);
            $stmt->execute(array($club_id, $to_date));

            $sql = "DELETE FROM banking WHERE club_id = ? and r_time < ?";
            $stmt = $db->prepare($sql);
            $stmt->execute(array($club_id, $to_date));

            $sql = "DELETE FROM log_money WHERE club_id = ? and u_time < ?";
            $stmt = $db->prepare($sql);
            $stmt->execute(array($club_id, $to_date));

            $sql = "DELETE FROM `order` WHERE club_id = ? and buy_time < ?";
            $stmt = $db->prepare($sql);
            $stmt->execute(array($club_id, $to_date));
    ...
    case 'create_club':
        ...
        try {
            ...
            $sql = "insert into club_race (club_id, race_id, start_date) select ?, id, start_date from race where start_date >= date_add(now(), interval -7 day)";
            $stmt = $db->prepare($sql);
            $stmt->execute(array($insertId));
    ...
    case 'money_io_real_point':
        ...
        try {
            ...
            if ((int)$amount >= 0) {
                $sql = "INSERT INTO `log_money` (`old_money_real`, `new_money_real`, `old_money_service`, `new_money_service`, `type`, `memo`, `user_id`, `club_id`)
			 select money_real - ?, money_real, money_service, money_service, '임의 지급', '파트너 포인트 -> 회원 머니 임의 지급으로 인한 변동'	 , user_id, club_id from user_info where user_info.user_id = ?";
                $stmt = $db->prepare($sql);
                $stmt->execute(array((int)$amount, $user_id));
            } else {
                $sql = "INSERT INTO `log_money` (`old_money_real`, `new_money_real`, `old_money_service`, `new_money_service`, `type`, `memo`, `user_id`, `club_id`)
			 select money_real + ?, money_real, money_service, money_service, '임의 환수', ''파트너 포인트 -> 회원 머니 임의 환수로 인한 변동'	 , user_id, club_id from user_info where user_info.user_id = ?";
                $stmt = $db->prepare($sql);
                $stmt->execute(array(abs((int)$amount), $user_id));
            }

            $sql = "update user_info set money_point = money_point - ? where user_id = ?";
            $stmt = $db->prepare($sql);
            $stmt->execute(array((int)$amount, $partner_id));

            $sql = "INSERT INTO `log_money` (`old_money_real`, `new_money_real`, `old_money_service`, `new_money_service`, `type`, `memo`, `user_id`, `club_id`)
			 select money_real + ?, money_real, money_service, money_service, '임의 지급 환수' , '파트너 포인트 -> 회원 임의 지급/환수로 인한 변동'	 , user_id, club_id from user_info where user_info.user_id = ?";
            $stmt = $db->prepare($sql);
            $stmt->execute(array((int)$amount, $partner_id));
    ...
    case 'money_io_real':
        ...
        try {
            ...
            if ((int)$amount >= 0) {
                $sql = "INSERT INTO `log_money` (`old_money_real`, `new_money_real`, `old_money_service`, `new_money_service`, `type`, `memo`, `user_id`, `club_id`)
			 select money_real - ?, money_real, money_service, money_service, '임의 지급', '머니 임의 지급으로 인한 변동'	 , user_id, club_id from user_info where user_info.user_id = ?";
                $stmt = $db->prepare($sql);
                $stmt->execute(array((int)$amount, $user_id));
            } else {
                $sql = "INSERT INTO `log_money` (`old_money_real`, `new_money_real`, `old_money_service`, `new_money_service`, `type`, `memo`, `user_id`, `club_id`)
			 select money_real + ?, money_real, money_service, money_service, '임의 환수', '머니 임의 환수로 인한 변동'	 , user_id, club_id from user_info where user_info.user_id = ?";
                $stmt = $db->prepare($sql);
                $stmt->execute(array(abs((int)$amount), $user_id));
            }
            $db->commit();
    ...
    case 'money_io_service':
        ...
        try {
            ...
            if ((int)$amount >= 0) {
                $sql = "INSERT INTO `log_money` (`old_money_real`, `new_money_real`, `old_money_service`, `new_money_service`, `type`, `memo`, `user_id`, `club_id`)
			 select money_real, money_real, money_service  - ? , money_service, '임의 지급', '서비스 머니 임의 지급으로 인한 변동'	 , user_id, club_id from user_info where user_info.user_id = ?";
                $stmt = $db->prepare($sql);
                $stmt->execute(array((int)$amount, $user_id));
            } else {
                $sql = "INSERT INTO `log_money` (`old_money_real`, `new_money_real`, `old_money_service`, `new_money_service`, `type`, `memo`, `user_id`, `club_id`)
			 select money_real, money_real, money_service + ?, money_service, '임의 환수', '서비스 머니 임의 환수로 인한 변동'	 , user_id, club_id from user_info where user_info.user_id = ?";
                $stmt = $db->prepare($sql);
                $stmt->execute(array(abs((int)$amount), $user_id));
            }
            $db->commit();
        ...
        try {
            $sql = "UPDATE user_info AS u
				...
				GROUP BY user_id) AS cr ON u.user_id = cr.u_id SET 
				 u.money_service = u.money_service + @allin_service_money := ROUND(IF(
	@allin_service_max_offset := IF(u.allin_service_config = 'C', c.allin_service_max_offset, u.u_allin_service_max_offset) = 0, @crprofit := (IF(profit IS NULL, 0, profit) + IF (cut_money_all IS NULL, 0, cut_money_all)) * (@allin_service := IF(u.allin_service_config = 'C', c.allin_service, u.u_allin_service)/100), IF(@allin_service_max_offset := IF(u.allin_service_config = 'C', c.allin_service_max_offset, u.u_allin_service_max_offset) >= @crprofit := (IF(profit IS NULL, 0, profit) + IF (cut_money_all IS NULL, 0, cut_money_all)), @crprofit := (IF(profit IS NULL, 0, profit) + IF (cut_money_all IS NULL, 0, cut_money_all)) * (@allin_service := IF(u.allin_service_config = 'C', c.allin_service, u.u_allin_service)/100), @allin_service_max_offset * (@allin_service := IF(u.allin_service_config = 'C', c.allin_service, u.u_allin_service)/100))))
				WHERE u.club_id = ? AND u.allin_service_config != 'N' AND u.user_level < 50 AND o.bet_money_all > 0 AND (IF(profit IS NULL, 0, profit) + IF (cut_money_all IS NULL, 0, cut_money_all)) >= IF(u.allin_service_config = 'C', c.allin_service_min_offset, u.u_allin_service_min_offset)";
            //where u.club_id = ? and u.allin_service_config != 'N' and u.user_level < 50 and o.bet_money_all > 0 and (if(profit is null, 0, profit) + if (cut_money_all is null, 0, cut_money_all)) >= if(u.allin_service_config = 'C', c.allin_service_min_offset, u.u_allin_service_min_offset)
            $stmt = $db->prepare($sql);
