...
function get_race_result($database, $url, $casper, $race)
{
  ...
  foreach ($trs as $key => $tr) {
    ...
    // exit();
    if (count($tds) > 3) {
      if ($tds[2]->textContent == '【未発売】' || $tds[2]->textContent == '【全返還】' || $tds[2]->textContent == '') {
        continue;
      }
      print_r($tds[2]->textContent);
...
// $date_url = array('http://www.tab.com.au/racing/meetings/today/R', 'http://www.tab.com.au/racing/meetings/today/H', 'http://www.tab.com.au/racing/meetings/today/G');
...
if ($races) {
  foreach ($races as $i => $race) {
    // $url = 'http://www.tab.com.au/racing/' . $v->start_date . '/' . $v->place_name . '/' . str_replace($v->association_code . '_', '', $v->place_code) . '/' . strtoupper(str_replace('os', '', $v->association_code)) . '/' . $v->race_no;
    // echo $url . PHP_EOL;
