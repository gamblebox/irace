...
function make_results_set_BP($result)
{
    /* 	stdClass Object
        (
            [idx] => 2008160151
...
switch ($parameter) {
        ...
        while (true) {
            ...
            if (count($short_games) > 0) {
                foreach ($short_games as $short_game) {
                    ...
                    $url = 'https://bepick.net/api/get_more/nlotto_power/default/' . substr(str_replace('-', '', $short_game->start_date), 2, 6) . str_pad(($short_game->race_no + 1), 4, "0", STR_PAD_LEFT);
                    echo $url . PHP_EOL;
                    ...
                    $result = $result[0];
                    echo $short_game->race_no . '==' . $result->round . PHP_EOL;
                    ...
                    if ($short_game->own_race_no == $result->rownum) {
                        $race_id = $short_game->id;
                        $results_set = make_results_set_BP($result);
                        $results_set = base_convert($results_set, 2, 10);
                        $balls = array($result->b1, $result->b2, $result->b3, $result->b4, $result->b5, $result->pb);
                        ...
                        echo interpolateQuery($sql, array($results_set, $results_contents, $race_id, $own_race_no, $race_id)) . PHP_EOL;;
                        $stmt = $db->prepare($sql);
                        ...
                        echo interpolateQuery($sql, array($race_id)) . PHP_EOL;;
                        $stmt = $db->prepare($sql);
    ...
    case 'close':
        while (true) {
            ...
            if (count($close_game) > 0) {
                ...
                do {
                    $count++;
                    $json = file_get_contents('https://bepick.net/api/get_more/nlotto_power/default/' . substr($day, 2, 6) . $race_no);
                    $result = json_decode($json);
    ...
    case 'day':
        $i = 289;
        while (true) {
            $url = 'https://bepick.net/api/get_more/nlotto_power/default/' . substr(date('Ymd'), 2, 6) . str_pad($i, 4, "0", STR_PAD_LEFT);
            echo $url . PHP_EOL;
            ...
            foreach ($results as $result) {
                ...
                $results_contents = json_encode($balls, JSON_UNESCAPED_UNICODE);
                print_r($results_contents);
                $own_race_no = $result->rownum;
                $race_no = $result->round;
                $sql = "INSERT INTO powerball_result ( results_set, results_contents, race_id, own_race_no, race_no, stat ) SELECT ?, ?, (select id from race where own_race_no = ?), ?, ?, 'E' FROM DUAL WHERE NOT EXISTS ( SELECT * FROM powerball_result WHERE own_race_no = ? )";
                $ok = interpolateQuery($sql, array($results_set, $results_contents, $own_race_no, $own_race_no, $race_no, $own_race_no));
                echo $ok . PHP_EOL;
                $stmt = $db->prepare($sql);
            ...
            if ($results[count($results) - 1]->date != date('Ymd')) {
                break;
            }
            $i = $results[count($results) - 1]->round;
        }
