...
function get_race_data_to_json($url)
{
	...
	foreach ($xpath->query('tr', $tbody[0]) as $index => $node) {
		...
		$rowData[7] = substr($rowData[7], 0, -3);
		unset($rowData[1]);
		unset($rowData[4]);
		unset($rowData[6]);
		unset($rowData[8]);
		...
		// count entry count
		// 			//$url = 'http://race.kra.co.kr/chulmainfo/chulmaDetailInfoChulmapyo.do?rcDate=20190727&rcNo=1&Sub=1&Act=02&meet=1';
		// 			$url = 'http://race.kra.co.kr/chulmainfo/chulmaDetailInfoChulmapyo.do?rcDate=' . str_replace('-', '', substr($rowData[1], 0, 10)) . '&rcNo=' . $rowData[2] . '&Sub=1&Act=02&meet=' . substr($rowData[0], 2, 1);
		// 			echo $url . PHP_EOL;
...
// $alinks = ['http://race.kra.co.kr/chulmainfo/ChulmaDetailInfoList.do?Act=02&Sub=1&meet=2'	];
