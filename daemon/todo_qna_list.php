...
Editor::inst($db, 'bbs_qna')
	->fields(
		//Field::inst( 'banking_type' ),
		Field::inst('bbs_qna.user_id')->validator('Validate::notEmpty')
			->options('user_info', 'user_id', array('nick_name', 'club_user_id'), function ($q) {
				global $club_id;
				$q->where('club_id', $club_id, '=');
			}, function ($row) {
				return $row['nick_name'] . ' (' . $row['club_user_id'] . ')';
			}),
		//Field::inst( 'club_id' ),
		...
		Field::inst('user_info.user_class_id'),
		Field::inst('bbs_qna.subject'),
		Field::inst('bbs_qna.u_time'),
		Field::inst('bbs_qna.memo_q'),
		Field::inst('bbs_qna.memo_a'),
		Field::inst('bbs_qna.boilerplate_id')->options('boilerplate', 'id', array('subject', 'memo'), function ($q) {
			global $club_id;
		...
		Field::inst('bbs_qna.type'),
		Field::inst('bbs_qna.stat')
	...
	->where('bbs_qna.club_id', $club_id, ($club_id === '255' ? '!=' : '='))
	->where('bbs_qna.stat', 'R', '=')
	->where('bbs_qna.u_time', date('Y-m-d', strtotime(date('Y-m-d') . '-2 days')), '>=')
