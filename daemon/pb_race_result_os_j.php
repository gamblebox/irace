...
function get_race_data_to_json($race)
{
	...
	for ($i = 1; $i < $race->entry_count + 1; $i++) {
		if (array_search($i, $cancel_entry_no_arr) === FALSE) {
	...
	// $permutations = new drupol\phpermutations\Generators\Combinations(array(1,2,3,4,5,6,7,8), 4);
	...
	$place3 = trim($place[3]);
	$place4 = trim($place[4]);
	$pb_place1 = trim($pb_place[1]);
	$pb_place2 = trim($pb_place[2]);
	$pb_place3 = trim($pb_place[3]);
	$pb_place4 = trim($pb_place[4]);
	print_r($pb_place);

	if (count($pb_place) > 4) {
		$over_count = true;
	} else if (count($pb_place) < 4) {
		$under_count = true;
	}
	if ($pow_count != 1) {
		$pow_no = 0;
	}
	$sum = array_sum($pb_place);
	$nb_oe = $sum % 2;
	if ($sum < $uo_line) {
		$uo = 1;
	} else {
	...
	// if ($race->pb_stat != 'N' && $race->entry_count > 6) {
	// 	if (! $over_count && ! $under_count && ! $under_min_entry_count) {
	// 		$sql = "INSERT INTO `pb_result` (`pb_race_id`, `pb_place_1`, `pb_place_2`, `pb_place_3`, `pb_place_4`, `pb_place_p`, `pb_place_sum`, `pb_place_oe`, `pb_place_uo`, `pb_place_p_oe`, `pb_place_p_uo`, `pb_place_uo_line`, `pb_place_p_uo_line`, `pb_results_set`, `pb_stat`) SELECT " .
	// 			$race->id . ", " . $pb_place1 . ", " . $pb_place2 . ", " . $pb_place3 . ", " . $pb_place4 . ", " . $pow_no . ", " . $sum . ", " . $nb_oe . ", " . $uo . ", " . $p_oe . ", " . $p_uo . ", " . $uo_line . ", " . $p_uo_line .
