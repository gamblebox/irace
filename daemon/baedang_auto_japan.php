...
{
    //https://keiba.rakuten.co.jp/race_card/list/RACEID/201806252015060112/mode/2
    //http://www2.keiba.go.jp/KeibaWeb/TodayRaceInfo/OddsUmLenFuku?k_raceDate=2018%2f06%2f25&k_raceNo=12&k_babaCode=20
    print_r($race);
    $url_rk = 'https://keiba.rakuten.co.jp/race_card/list/RACEID/' . str_replace('-', '', $race->start_date) . sprintf("%02d", $race->own_id) . $race->rk_race_code . sprintf("%02d", $race->race_no) . '/mode/2';
    $url = 'http://www2.keiba.go.jp/KeibaWeb/TodayRaceInfo/OddsUmLenFuku?k_raceDate=' . str_replace('-', '%2f', $race->start_date) . '&k_raceNo=' . $race->race_no . '&k_babaCode=' . $race->own_id;
    ...
    // ));
    //     $casper->start($url);
    //     $casper->waitForText('</noscript>', 3000);
    //     $casper->run();
    //     $html = $casper->getHtml();
    // echo $wp_html;
    $html_rk = file_get_contents($url_rk);
    //     echo $html_rk;

    $dom = HtmlDomParser::str_get_html($html_rk);
    //     print_r($dom);
    $trs = $dom->find('div')[0]->plaintext;
    print_r($trs);

    return;
    //     echo '$trs ' . $trs . PHP_EOL;
    if ($page == 'Uh Oh!') {
        return;
    }
...
// while (true) {
// code...
//    $sql = "SELECT r.id, r.association_code, r.place_name, r.place_code, r.start_date, r.start_time, r.race_no, p.name as place_name FROM race r left join place p on r.place_id = p.id WHERE start_time < date_add(now(), INTERVAL " . $interval . ") and start_time > date_add(now(), INTERVAL -10 minute) and r.association_code in ('japanrace') and r.stat = 'E' order by r.start_time asc;";
$sql = "SELECT r.id, r.association_code, r.place_name, r.rk_race_code, p.own_id, r.start_date, r.start_time, r.race_no, p.name as place_name FROM race r left join place p on r.place_id = p.id WHERE start_time < date_add(now(), INTERVAL " . $interval . ") and start_time > date_add(now(), INTERVAL -2 hour) and r.association_code in ('japanrace') order by r.start_time asc limit 1;";
